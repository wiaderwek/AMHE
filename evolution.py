...
def reproduce_members(first_member, second_member):
    first_shortes, second_shortest = reproduce_path(first_member[SHORTEST], second_member[SHORTEST])
    first_best, second_best = reproduce_path(first_member[BEST], second_member[BEST])

    return {SHORTEST: first_shortes, BEST: first_best}, {SHORTEST: second_shortest, BEST: second_best}
...
def mutate(reproduced_members):
    for i in range(len(reproduced_members) - 1):
        if random.randint(0, 100) <= MUTATE_MEMBER_PROBABILITY:
            reproduced_members[i][SHORTEST] = mutate_path(reproduced_members[i][SHORTEST])
